object FormEmprestimos: TFormEmprestimos
  Left = 371
  Top = 188
  BorderIcons = [biSystemMenu]
  BorderStyle = bsSingle
  Caption = 'Registro de Empr'#233'stimos - Biblioteca'
  ClientHeight = 394
  ClientWidth = 456
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  FormStyle = fsMDIChild
  OldCreateOrder = False
  Position = poMainFormCenter
  Visible = True
  OnClose = FormClose
  OnCreate = FormCreate
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object PageControl: TPageControl
    Left = 0
    Top = 0
    Width = 456
    Height = 355
    ActivePage = TabSheetListagem
    Align = alClient
    TabOrder = 0
    OnChange = PageControlChange
    object TabSheetListagem: TTabSheet
      Caption = 'Listagem'
      object PanelFiltro: TPanel
        Left = 0
        Top = 0
        Width = 448
        Height = 65
        Align = alTop
        BevelInner = bvLowered
        TabOrder = 0
        DesignSize = (
          448
          65)
        object BtnPesquisar: TBitBtn
          Left = 333
          Top = 8
          Width = 107
          Height = 49
          Anchors = [akTop, akRight]
          Caption = '&Pesquisar'
          TabOrder = 0
          OnClick = BtnPesquisarClick
          Glyph.Data = {
            
            C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0}
        end
        object GroupBoxPeriodo: TGroupBox
          Left = 8
          Top = 8
          Width = 265
          Height = 49
          Caption = 'Per'#237'odo'
          TabOrder = 1
          object DateEditIni: TDateEdit
            Left = 8
            Top = 16
            Width = 121
            Height = 21
            NumGlyphs = 2
            StartOfWeek = Sun
            TabOrder = 0
          end
          object DateEditFim: TDateEdit
            Left = 136
            Top = 16
            Width = 121
            Height = 21
            NumGlyphs = 2
            StartOfWeek = Sun
            TabOrder = 1
          end
        end
      end
      object PanelGrid: TPanel
        Left = 0
        Top = 65
        Width = 448
        Height = 262
        Align = alClient
        BevelInner = bvLowered
        TabOrder = 1
        object DBGrid: TRxDBGrid
          Left = 2
          Top = 2
          Width = 444
          Height = 258
          Align = alClient
          DataSource = Ds
          Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
          TabOrder = 0
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clWindowText
          TitleFont.Height = -11
          TitleFont.Name = 'MS Sans Serif'
          TitleFont.Style = []
          OnDblClick = DBGridDblClick
          Columns = <
            item
              Expanded = False
              FieldName = 'Data'
              Width = 103
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'NomeMembro'
              Title.Caption = 'Membro'
              Width = 298
              Visible = True
            end>
        end
      end
    end
    object TabSheetManutencao: TTabSheet
      Caption = 'Manuten'#231#227'o'
      ImageIndex = 1
      object PanelDados: TPanel
        Left = 0
        Top = 0
        Width = 448
        Height = 327
        Align = alClient
        BevelInner = bvLowered
        Enabled = False
        TabOrder = 0
        object Label3: TLabel
          Left = 8
          Top = 8
          Width = 33
          Height = 13
          Caption = 'C'#243'digo'
          FocusControl = DBEdit1
        end
        object Label7: TLabel
          Left = 8
          Top = 48
          Width = 38
          Height = 13
          Caption = 'Membro'
        end
        object SpdBtnMembro: TSpeedButton
          Left = 84
          Top = 63
          Width = 23
          Height = 22
          Flat = True
          Glyph.Data = {
            
            D0D4C8D0D4C8D0D4C8D0D4C8D0D4C8D0D4C8D0D4C8D0D4C8D0D4}
          OnClick = SpdBtnMembroClick
        end
        object DBText2: TDBText
          Left = 113
          Top = 71
          Width = 50
          Height = 13
          AutoSize = True
          DataField = 'NomeMembro'
          DataSource = Ds
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clHotLight
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label5: TLabel
          Left = 8
          Top = 88
          Width = 54
          Height = 13
          Caption = 'Empr'#233'stimo'
        end
        object Label6: TLabel
          Left = 136
          Top = 88
          Width = 52
          Height = 13
          Caption = 'Devolu'#231#227'o'
        end
        object Label8: TLabel
          Left = 272
          Top = 88
          Width = 26
          Height = 13
          Caption = 'Baixa'
        end
        object DBText3: TDBText
          Left = 273
          Top = 111
          Width = 50
          Height = 13
          AutoSize = True
          DataField = 'Baixa'
          DataSource = Ds
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clHotLight
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object DBEdit1: TDBEdit
          Left = 8
          Top = 24
          Width = 133
          Height = 21
          CharCase = ecUpperCase
          Color = 16776176
          DataField = 'Codigo'
          DataSource = Ds
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
          TabOrder = 0
        end
        object DBEdit3: TDBEdit
          Left = 8
          Top = 64
          Width = 73
          Height = 21
          CharCase = ecUpperCase
          DataField = 'Membro'
          DataSource = Ds
          TabOrder = 1
          OnKeyDown = DBEdit3KeyDown
        end
        object DBDateEdit1: TDBDateEdit
          Left = 8
          Top = 104
          Width = 121
          Height = 21
          DataField = 'Data'
          DataSource = Ds
          NumGlyphs = 2
          TabOrder = 2
          StartOfWeek = Sun
        end
        object DBDateEdit2: TDBDateEdit
          Left = 136
          Top = 104
          Width = 121
          Height = 21
          DataField = 'Devolucao'
          DataSource = Ds
          NumGlyphs = 2
          TabOrder = 3
          StartOfWeek = Sun
        end
        object GroupBoxItens: TGroupBox
          Left = 7
          Top = 130
          Width = 434
          Height = 191
          Caption = 'Itens do Empr'#233'stimo'
          TabOrder = 4
          DesignSize = (
            434
            191)
          object Label1: TLabel
            Left = 8
            Top = 16
            Width = 31
            Height = 13
            Caption = 'Objeto'
          end
          object SpdBtnObjetos: TSpeedButton
            Left = 68
            Top = 31
            Width = 23
            Height = 22
            Flat = True
            Glyph.Data = {
              
              D0D4C8D0D4C8D0D4C8D0D4C8D0D4C8D0D4C8D0D4C8D0D4C8D0D4}
            OnClick = SpdBtnObjetosClick
          end
          object LblTituloObjeto: TLabel
            Left = 96
            Top = 39
            Width = 5
            Height = 13
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clNavy
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = [fsBold]
            ParentFont = False
          end
          object BtnAddItens: TBitBtn
            Left = 393
            Top = 28
            Width = 33
            Height = 25
            Anchors = [akTop, akRight]
            TabOrder = 1
            OnClick = BtnAddItensClick
            Glyph.Data = {
              
              8888880BB088888888888800008888888888}
          end
          object EditObjeto: TEdit
            Left = 8
            Top = 32
            Width = 57
            Height = 21
            CharCase = ecUpperCase
            MaxLength = 8
            TabOrder = 0
            OnEnter = EditObjetoEnter
            OnExit = EditObjetoExit
            OnKeyDown = EditObjetoKeyDown
            OnKeyPress = EditCEPKeyPress
          end
        end
      end
      object DBGridItens: TRxDBGrid
        Left = 15
        Top = 186
        Width = 417
        Height = 129
        DataSource = DsItens
        Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
        PopupMenu = PopupMenuItens
        TabOrder = 1
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'MS Sans Serif'
        TitleFont.Style = []
        Columns = <
          item
            Expanded = False
            FieldName = 'Objeto'
            Title.Caption = 'C'#243'digo do Objeto'
            Width = 89
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'TituloObjeto'
            Width = 287
            Visible = True
          end>
      end
    end
  end
  object PanelBotoes: TPanel
    Left = 0
    Top = 355
    Width = 456
    Height = 39
    Align = alBottom
    BevelInner = bvLowered
    TabOrder = 1
    object ToolBar1: TToolBar
      Left = 2
      Top = 2
      Width = 452
      Height = 34
      ButtonHeight = 30
      ButtonWidth = 36
      Caption = 'ToolBar1'
      Flat = True
      Images = FormMain.ImageList24
      TabOrder = 0
      object ToolButton1: TToolButton
        Left = 0
        Top = 0
        Action = DataSetPrior1
        ParentShowHint = False
        ShowHint = True
      end
      object ToolButton2: TToolButton
        Left = 36
        Top = 0
        Action = DataSetNext1
        ParentShowHint = False
        ShowHint = True
      end
      object ToolButton5: TToolButton
        Left = 72
        Top = 0
        Width = 8
        Caption = 'ToolButton5'
        ImageIndex = 4
        Style = tbsSeparator
      end
      object ToolButton6: TToolButton
        Left = 80
        Top = 0
        Action = DataSetInsert1
        ParentShowHint = False
        ShowHint = True
      end
      object ToolButton7: TToolButton
        Left = 116
        Top = 0
        Action = DataSetEdit1
        ParentShowHint = False
        ShowHint = True
      end
      object ToolButton8: TToolButton
        Left = 152
        Top = 0
        Action = DataSetDelete1
        ParentShowHint = False
        ShowHint = True
      end
      object ToolButton9: TToolButton
        Left = 188
        Top = 0
        Width = 8
        Caption = 'ToolButton9'
        ImageIndex = 7
        Style = tbsSeparator
      end
      object ToolButton10: TToolButton
        Left = 196
        Top = 0
        Action = DataSetPost1
        ParentShowHint = False
        ShowHint = True
      end
      object ToolButton11: TToolButton
        Left = 232
        Top = 0
        Action = DataSetCancel1
        ParentShowHint = False
        ShowHint = True
      end
      object ToolButton3: TToolButton
        Left = 268
        Top = 0
        Width = 8
        Caption = 'ToolButton3'
        ImageIndex = 6
        Style = tbsSeparator
      end
      object ToolButton13: TToolButton
        Left = 276
        Top = 0
        Hint = 'Sair'
        Caption = 'ToolButton13'
        ImageIndex = 5
        ParentShowHint = False
        ShowHint = True
        OnClick = ToolButton13Click
      end
    end
  end
  object Ds: TDataSource
    DataSet = QryEmprestimos
    Left = 288
  end
  object ActionList1: TActionList
    Images = FormMain.ImageList24
    Left = 260
    object DataSetPrior1: TDataSetPrior
      Category = 'Dataset'
      Caption = 'DataSetPrior1'
      Hint = 'Voltar'
      ImageIndex = 8
      DataSource = Ds
    end
    object DataSetNext1: TDataSetNext
      Category = 'Dataset'
      Caption = 'DataSetNext1'
      Hint = 'Avan'#231'ar'
      ImageIndex = 3
      DataSource = Ds
    end
    object DataSetInsert1: TDataSetInsert
      Category = 'Dataset'
      Caption = 'DataSetInsert1'
      Hint = 'Incluir'
      ImageIndex = 9
      DataSource = Ds
    end
    object DataSetDelete1: TDataSetDelete
      Category = 'Dataset'
      Caption = 'DataSetDelete1'
      Hint = 'Excluir'
      ImageIndex = 2
      DataSource = Ds
    end
    object DataSetEdit1: TDataSetEdit
      Category = 'Dataset'
      Caption = 'DataSetEdit1'
      Hint = 'Alterar'
      ImageIndex = 0
      DataSource = Ds
    end
    object DataSetPost1: TDataSetPost
      Category = 'Dataset'
      Caption = 'DataSetPost1'
      Hint = 'Salvar'
      ImageIndex = 11
      DataSource = Ds
    end
    object DataSetCancel1: TDataSetCancel
      Category = 'Dataset'
      Caption = 'DataSetCancel1'
      Hint = 'Cancelar'
      ImageIndex = 7
      DataSource = Ds
    end
  end
  object QryEmprestimos: TUniQuery
    Connection = DMJetro.Conn
    SQL.Strings = (
      'select * from Emprestimos'
      'where codigo = -1')
    DMLRefresh = True
    BeforeInsert = QryEmprestimosBeforeInsert
    BeforeEdit = QryEmprestimosBeforeEdit
    BeforePost = QryEmprestimosBeforePost
    AfterPost = QryEmprestimosAfterPost
    AfterCancel = QryEmprestimosAfterCancel
    BeforeDelete = QryEmprestimosBeforeDelete
    AfterScroll = QryEmprestimosAfterScroll
    OnCalcFields = QryEmprestimosCalcFields
    OnNewRecord = QryEmprestimosNewRecord
    Left = 232
    object QryEmprestimosCodigo: TIntegerField
      FieldName = 'Codigo'
      ProviderFlags = [pfInWhere]
      ReadOnly = True
    end
    object QryEmprestimosMembro: TIntegerField
      DisplayLabel = 'C'#243'digo do Membro'
      FieldName = 'Membro'
      Required = True
    end
    object QryEmprestimosData: TDateTimeField
      DisplayLabel = 'Data de Empr'#233'stimo'
      FieldName = 'Data'
      Required = True
    end
    object QryEmprestimosDevolucao: TDateTimeField
      DisplayLabel = 'Data da Devoluc'#227'o'
      FieldName = 'Devolucao'
      Required = True
    end
    object QryEmprestimosBaixa: TDateTimeField
      FieldName = 'Baixa'
    end
    object QryEmprestimosFechamento: TStringField
      FieldName = 'Fechamento'
      Size = 1
    end
    object QryEmprestimosUserInsert: TIntegerField
      FieldName = 'UserInsert'
    end
    object QryEmprestimosDataInsert: TDateTimeField
      FieldName = 'DataInsert'
    end
    object QryEmprestimosUserUpdate: TIntegerField
      FieldName = 'UserUpdate'
    end
    object QryEmprestimosDataUpdate: TDateTimeField
      FieldName = 'DataUpdate'
    end
    object QryEmprestimosTituloObjeto: TStringField
      FieldKind = fkCalculated
      FieldName = 'TituloObjeto'
      Size = 50
      Calculated = True
    end
    object QryEmprestimosNomeMembro: TStringField
      FieldKind = fkCalculated
      FieldName = 'NomeMembro'
      Size = 50
      Calculated = True
    end
  end
  object QryItens: TUniQuery
    Connection = DMJetro.Conn
    SQL.Strings = (
      'select * from ItensEmprestimo'
      'where Emprestimo = -1')
    DMLRefresh = True
    BeforeDelete = QryItensBeforeDelete
    OnCalcFields = QryItensCalcFields
    Left = 360
    Top = 8
    object QryItensCodigo: TIntegerField
      FieldName = 'Codigo'
      ProviderFlags = [pfInWhere]
      ReadOnly = True
    end
    object QryItensEmprestimo: TIntegerField
      FieldName = 'Emprestimo'
    end
    object QryItensObjeto: TIntegerField
      FieldName = 'Objeto'
    end
    object QryItensTituloObjeto: TStringField
      DisplayLabel = 'T'#237'tulo do Objeto'
      FieldKind = fkCalculated
      FieldName = 'TituloObjeto'
      Size = 50
      Calculated = True
    end
  end
  object DsItens: TDataSource
    DataSet = QryItens
    Left = 392
    Top = 8
  end
  object PopupMenuItens: TPopupMenu
    OnPopup = PopupMenuItensPopup
    Left = 372
    Top = 272
    object Excluir1: TMenuItem
      Caption = 'Excluir Item'
      OnClick = Excluir1Click
    end
  end
end
